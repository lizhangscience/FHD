FUNCTION transfer_calibration, obs, params, transfer_cal,
    file_path_fhd=file_path_fhd,silent=silent,error=error,_Extra=extra
IF size(transfer_cal,/type) EQ 7 THEN BEGIN
  cal_file_use=transfer_cal
  IF file_test(cal_file_use,/directory) THEN BEGIN
    fhd_save_io,file_path_fhd=file_path_fhd,transfer=transfer_cal,var='cal',path_use=cal_file_use2,_Extra=extra
    cal_file_use2+='.sav'
